...
test_that("'as_utf8' errors on latin1 declared to be UTF-8", {
    ...
    expect_error(as_utf8(x), "argument entry 3 is marked as \"UTF-8\" but contains an invalid byte in position 36 (\\xa3)", fixed = TRUE)
})
...
test_that("utf8_valid errors on invalid UTF-8", {
    ...
    expect_error(as_utf8(x), "argument entry 5 is marked as \"UTF-8\" but contains an invalid byte in position 1 (\\xf8)", fixed = TRUE)
})
