...
test_that("'as_utf8' errors on latin1 declared to be UTF-8", {
    ...
    expect_error(as_utf8(x), "argument entry 3 is incorrectly marked as \"UTF-8\": invalid leading byte (0xA3) at position 36", fixed = TRUE)
})
...
test_that("utf8_valid errors on invalid UTF-8", {
    ...
    expect_error(as_utf8(x), "argument entry 5 is incorrectly marked as \"UTF-8\": invalid leading byte (0xF8) at position 1", fixed = TRUE)
})
...
test_that("utf8_valid errors on invalid UTF8 in bytes encoding", {
    ...
    expect_error(as_utf8(x), "argument entry 1 cannot be cast from \"bytes\" to \"UTF-8\": invalid leading byte (0xFC) at position 6", fixed = TRUE)
})
